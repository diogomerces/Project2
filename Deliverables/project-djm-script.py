...
#####Create review df
review=comments_filter[["id","queryDate","tagline","review_body","program","school_id"]]
...
#review = review2.drop(columns = ['program','course_name','school_id_x'],  axis =1)

cols=['review_id', 'created_at', 'tag_line', 'review_body', 'course', 'school_id',"name_id","score_id"]
review.columns=cols

cols2=['review_id', 'created_at', 'tag_line', 'review_body', 'course',"name_id","score_id",'school_id']
review=review[cols2]
...
personal = personal2.drop(columns = ['id','review_id','created_at',"tag_line","review_body","score_id"],  axis =1)
...
score2 = score.merge(review, how = 'inner', left_on ="id", right_on = 'review_id')
...
location = location.drop(columns = ['id','country.abbrev','city.keyword','state.id','state.name','state.abbrev','state.keyword','school','school_id'],  axis =1)
...
location["location_id"]=location["description"].apply(generate_id)
#location = location.drop(columns = ['description'],  axis =1)

#ALterando a posição da coluna location_id
cols =["location_id",'country.id','country.name','city.id','city.name',"description"]
...
school_location = location.merge(location_bridge, how = 'inner', left_on ="description", right_on = 'description')

school_location = school_location.drop(columns = ['country_id', 'country_name', 'city_id', 'city_name',
       'description', 'id', 'country.id', 'country.name', 'country.abbrev',
       'city.id', 'city.name', 'city.keyword', 'state.id', 'state.name',
       'state.abbrev', 'state.keyword', 'school'],  axis =1)

school_location = school_location[['school_id','location_id']]
location = location.drop(columns = ['description'],  axis =1)
#########################
